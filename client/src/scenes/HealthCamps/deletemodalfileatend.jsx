...
import { useAddCampMutation, useUpdateCampMutation, useGetLastCampQuery } from "state/api";
...
const HealthCampModal = ({ openModal, closeModal, currentCamp }) => {
  ...
  useEffect(() => {
    if (currentCamp) {
      ...
      setMohFields(currentCamp.MOH);
      setContactPersons(currentCamp.ContactPersons);
      setSponsors(currentCamp.Sponsors);
    } else if (isSuccess && lastCamp) {
      setCampId(generateNextId(lastCamp.CampId));
    } else {
      setCampId("MD-HC-000001");
    }
  }, [currentCamp, lastCamp, isSuccess]);
  ...
  const handleSubmit = () => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      const submitAction = currentCamp ? updateCamp({ id: currentCamp._id, ...campData }) : addCamp(campData);
      ...
      submitAction
        .then((response) => {
          ...
          setTimeout(() => {
            setLoading(false);
            closeModal();
            setSnackbar({ open: true, message: `Health Camp ${currentCamp ? "updated" : "created"} successfully`, severity: "success" });
          }, remainingTime > 0 ? remainingTime : 0);
        })
        .catch((error) => {
          console.error(`Error ${currentCamp ? "updating" : "adding"} camp:`, error);
          setLoading(false);
          setSnackbar({ open: true, message: `Error ${currentCamp ? "updating" : "adding"} camp`, severity: "error" });
        });
    }
  };
    ...
    <>
      ...
      >
        ...
        <div style={{ color: "#d63333", fontWeight: '700', fontSize: '16px' }}>
          {currentCamp ? "Update Health Camp" : "Create Health Camp"}
